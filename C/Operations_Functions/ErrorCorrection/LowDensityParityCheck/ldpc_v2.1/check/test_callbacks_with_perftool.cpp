/* $Id: test_callbacks_with_perftool.cpp,v 1.7 2006/09/06 10:15:06 roca Exp $ */
...
//#define LDPC
#ifdef LDPC_STAIRS
	if (MyFecSession.InitSession(nbDATA, nbFEC, pktSize, FLAG_BOTH, rand(), TypeSTAIRS) == LDPC_ERROR)
#elif defined (LDPC_TRIANGLE)
	if (MyFecSession.InitSession(nbDATA, nbFEC, pktSize, FLAG_BOTH, rand(), TypeTRIANGLE) == LDPC_ERROR)
#elif defined (LDGM)
	if (MyFecSession.InitSession(nbDATA, nbFEC, pktSize, FLAG_BOTH, rand(), TypeLDGM, left_degree) == LDPC_ERROR)
#endif // codec
	{
	...
	for (fecseq = 0; fecseq < nbFEC; fecseq++) {
		MyFecSession.BuildParitySymbol((void**)dataOrig, fecseq,
	...
	for (decodeSteps = 0; decodeSteps < nbPKT; ) {
		...
		MyFecSession.DecodingStepWithSymbol((void**)dataDest, newPkt, newPktIdx, true);
		     ...
		     int	pkt_seqno)
{
	...
	if (verbosity > 0) {
		printf("DecodedPkt_callback: ctxt=%p, size=%d, seqno=%d\n",
			context, size, pkt_seqno);
	}
	return (void *)pkt;
}
			 ...
			 int	size)
{
	...
	if (verbosity > 0) {
		printf("AllocTmpBuffer_callback: ctxt=%p, size=%d\n",
			context, size);
	}
	return (void *)pkt;
}
		  ...
		  void	*pkt)
{
	...
	if (verbosity > 0) {
		printf("GetData_callback: ctxt=%p, pkt=%p\n",
			context, pkt);
	}
		  ...
		  void	*pkt)
{
	...
	if (verbosity > 0) {
		printf("GetDataPtrOnly_callback: ctxt=%p, pkt=%p\n",
			context, pkt);
	}
	return (((pkt_t*)pkt)->buf);
}
		    ...
		    void	*pkt)
{
	...
	if (verbosity > 0) {
		printf("StoreData_callback: ctxt=%p, pkt=%p\n",
			context, pkt);
	}
	return LDPC_OK;
}
		  ...
		  void		*pkt)
{
	...
	free(pkt);
	if (verbosity > 0) {
		printf("FreePkt_callback: ctxt=%p, pkt=%p\n",
			context, pkt);
	}
	return LDPC_OK;
}
